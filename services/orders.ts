...
export const createOrderOnMP = async (
  order: Order
  ...
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Failed to create order on Mercado Pago"
    );
  }
};

export const getOrderOnMP = async (
  mpOrderId: string
): Promise<MPOrderResponse> => {
  const token = process.env.MERCADO_PAGO_ACCESS_TOKEN;
  if (!token) throw new Error("Missing Mercado Pago access token");
  ...
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Failed to fetch order from Mercado Pago"
    );
  }
};
