...
app.post("/api/menu", (req, res) => {
    ...
    if (!name || typeof price !== "number") {
        return res.status(400).json({
            error: 'Invalid menu item. "name" (string) and "price" (number) are required.',
        });
    }
...
app.post("/api/orders", (req, res) => {
    ...
    if (!Array.isArray(items) || typeof total !== "number") {
        return res.status(400).json({
            error: 'Invalid order. "items" (array) and "total" (number) are required.',
        });
    }
    const newOrder = {
        id: `ORDER-${Date.now()}`,
...
console.log("Initializing Mercado Pago with token:", process.env.MERCADO_PAGO_ACCESS_TOKEN?.substring(0, 10) + "...");
...
const testMercadoPago = async () => {
    try {
        ...
        console.log("Using token:", process.env.MERCADO_PAGO_ACCESS_TOKEN?.substring(0, 15) + "...");
        let testPayment;
        try {
            if (payment && typeof payment.create === "function") {
                testPayment = await payment.create(payload);
                console.log("Raw test payment response (SDK):", JSON.stringify(testPayment, null, 2));
                testPayment = testPayment?.response || testPayment;
            }
            else {
                testPayment = await createPaymentDirect(payload);
                console.log("Raw test payment response (REST):", JSON.stringify(testPayment, null, 2));
            }
            console.log("Normalized test response body:", JSON.stringify(testPayment, null, 2));
            return true;
        }
        catch (error) {
            ...
            try {
                const tokenInfo = await axios.get("https://api.mercadopago.com/users/me", {
                    headers: {
